...
void repListing(orderedLinkedList<bookType*> &bookInfo)
{
  ...
  do
  {
    ...
    int pagePadding = REP_LISTING_WIDTH - (12 + 6 + to_string(currentPage).length() + 4 + to_string(totalPages).length() + 16 + to_string(bookInfo.length()).length() * 2 + 32);
...
void repWholesale(orderedLinkedList<bookType*> &bookInfo)
{
  ...
  do
  {
    ...
    int pagePadding = REP_LISTING_WIDTH - (12 + 6 + to_string(currentPage).length() + 4 + to_string(totalPages).length() + 11);
    cout << "█  Date: " << left << setw(12) << date 
         << "PAGE: " << currentPage << " of " << totalPages 
         << setw(pagePadding) << " " << "█" << endl;
